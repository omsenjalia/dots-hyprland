...
**Deployment**: Automatic via Vercel Git integration (auto-deploys on push to `main`). No explicit GitHub Actions workflow — Vercel watches the repo directly. Build config is in `docs/vercel.json`.

**Upstream sync rule**: Before completing any work session, check [ii.clsty.link](https://ii.clsty.link/en/) for new upstream content that should be reflected in our docs. See `docs/src/content/docs/ai-agents/03-update-docs.md` for the full upstream sync workflow.
